        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="text-4xl font-extrabold mb-4">{{ $curiosidad->title }}</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">{{ __('Publicado el') }} {{ $curiosidad->created_at ? $curiosidad->created_at->format('d M, Y') : 'Fecha no disponible' }}</p>
                    
                    <img class="w-full h-80 object-cover mb-8 rounded-lg" 
                        <?php
                            $imageUrl = $curiosidad->image_url;
                            if ($imageUrl && (str_starts_with($imageUrl, 'http://') || str_starts_with($imageUrl, 'https://'))) {
                                echo 'src="' . e($imageUrl) . '"';
                            } elseif ($imageUrl) {
                                echo 'src="' . e(asset('storage/' . $imageUrl)) . '"';
                            } else {
                                echo 'src="https://placehold.co/1200x400/333333/FFFFFF/png?text=Curiosity+Image"';
                            }
                        ?>
                    alt="{{ $curiosidad->title }}">
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mb-10 p-6 bg-gray-50 dark:bg-gray-800 rounded-lg shadow-inner">
                            ...
                            <form action="{{ route('comments.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="commentable_type" value="App\Models\Curiosidad">
                                <input type="hidden" name="commentable_id" value="{{ $curiosidad->id }}">
