<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-100 leading-tight">Crear Evento Biográfico</h2></x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 border border-gray-700 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('admin.biografia-eventos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    <div><label class="block text-sm font-medium text-gray-300">Año</label><input type="number" name="year" class="mt-1 block w-full bg-gray-900 border-gray-600 text-white rounded-md" required min="1900" max="2100"></div>
